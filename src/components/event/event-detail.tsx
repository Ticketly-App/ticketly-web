...
import { useTicketlyEvent, useTicketlyEvents, mapTicketTier } from '@/hooks/use-ticketly-events'
...
export function EventDetail({ eventKey }: { eventKey: string }) {
  ...
  // Organizer profile popup state
  const [orgPopup, setOrgPopup] = useState<{ authority: string } | null>(null)

  // Organizer profiles hook
  const { data: organizerProfiles, isLoading: orgProfilesLoading } = useOrganizerProfiles(event ? [event.authority] : [])

  // Organizer profile lookup
  const orgProfile = event && organizerProfiles ? organizerProfiles.get(event.authority) : undefined
  const orgLogo = orgProfile?.logoUri || ''
  const orgHandle = orgProfile?.username || ''
  const orgName = orgProfile?.name || ''

  // Helper for event object in button
  const ev = event
  ...
  const handleMint = async () => {
    ...
    if (activeTier.saleNotStarted) {
      toast.error(`Sale starts ${new Date(Number(activeTier.saleStart) * 1000).toLocaleString()}`)
      return
    }
    if (activeTier.saleEnded) {
      toast.error('Sale for this tier has ended.')
      return
    }
  ...
  return (
    <XAuthGate message="Verify your X (Twitter) account to browse events and purchase tickets on Ticketly.">
      <div className="min-h-screen bg-dark-950">
        <Navbar />
        ...
        <section className="relative pt-24 pb-12 px-6">
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-brand-900/8 rounded-full blur-[150px]" />
          </div>
          <div className="relative max-w-6xl mx-auto">
            <div className="flex items-center gap-3 mb-6">
              ...
              <span
                className={`badge ${event.isCancelled ? 'bg-red-500/20 text-red-400 border-red-500/30' : event.isActive ? 'badge-active' : 'bg-white/10 text-white/50 border-white/10'}`}
              >
                {event.isCancelled ? 'Cancelled' : event.isActive ? 'On Sale' : 'Draft'}
              </span>
              {event.poapEnabled && <span className="badge badge-vip">POAP</span>}
              {event.resaleAllowed && (
                <span className="badge bg-cyan-500/20 text-cyan-400 border-cyan-500/30">Resale</span>
              )}
              {event.whitelistGated && (
                <span className="badge bg-amber-500/20 text-amber-400 border-amber-500/30">Whitelist</span>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-6 text-sm">
              ...
              <div className="flex items-center gap-2 text-white/60">
                ...
                {eventStart.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })} –{' '}
                {eventEnd.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}
              </div>
              <div className="flex items-center gap-2 text-white/60">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <section className="px-6 pb-20">
          <div className="max-w-6xl mx-auto grid gap-8 lg:grid-cols-[1fr_400px]">
                ...
                {tiers.map((tier, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left ticket-card p-5 transition-all ${isSelected ? 'border-brand-500/30 bg-brand-600/5' : 'hover:bg-white/02'} ${tier.soldOut ? 'opacity-50' : ''}`}
                      disabled={tier.soldOut}
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${isSelected ? 'border-brand-400' : 'border-white/20'}`}
                          >
                      ...
                      <div className="flex items-center gap-4 text-xs text-white/40">
                        <span>
                          {tier.minted}/{tier.supply} minted
                        </span>
                        <span>{tier.available} remaining</span>
                        {tier.saleNotStarted && (
                          <span className="text-amber-400">
                            Sale starts {new Date(Number(tier.saleStart) * 1000).toLocaleString()}
                          </span>
                        )}
                        {tier.saleEnded && <span className="text-red-400">Sale ended</span>}
                      </div>
              ...
              <div className="glass rounded-2xl p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    {
                      label: 'Revenue',
                      value: `${lamportsToSol(Number(event.totalRevenue)).toFixed(4)} SOL`,
                      accent: true,
                    },
            ...
            <div className="lg:sticky lg:top-28 h-fit">
              <div className="glass-strong rounded-2xl p-6 border border-brand-500/20 space-y-5">
                <div>
                  <h3 className="font-display text-lg text-white">Buy Ticket</h3>
                  <p className="text-xs text-white/40 mt-1">
                    Mint an NFT ticket directly to your wallet on Solana devnet.
                  </p>
                </div>
                {activeTier && (
                  <div className="glass rounded-xl p-4 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-medium text-white">
                        {TIER_NAMES[activeTier.tierType] || `Tier ${selectedTier}`}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-white/60">Price</span>
                      <span className="font-display text-brand-400">
                        {activeTier.priceSol === 0 ? 'FREE' : `${activeTier.priceSol.toFixed(4)} SOL`}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-white/60">Available</span>
                      <span className="text-sm text-white">
                        {activeTier.available} / {activeTier.supply}
                      </span>
                    </div>
                  </div>
                )}
                <div className="flex items-center justify-between pt-2 border-t border-white/08">
                  ...
                  <span className="font-display text-xl text-white">
                    {activeTier ? (activeTier.priceSol === 0 ? 'FREE' : `${activeTier.priceSol.toFixed(4)} SOL`) : '—'}
                  </span>
                </div>
                <button
                  onClick={handleMint}
                  disabled={
                    ...
                    !activeTier ||
                    activeTier.soldOut ||
                    activeTier.saleNotStarted ||
                    activeTier.saleEnded ||
                    event.isCancelled ||
                    !publicKey
                  }
                ...
                <p className="text-[11px] text-white/30 text-center">
                  Transaction is signed and confirmed on Solana devnet.
                </p>
              </div>
            </div>
          </div>
